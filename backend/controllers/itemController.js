...
itemController.post('/item', upload.single('image'), async (req, res) => {
    try {
        const { name, description, size, price, tag, favorite, seller_id, bought_id, userSell_id } = req.body;
...
itemController.put('/item/:id([0-9]+)', upload.single('image'), async (req, res) => {
    const { id } = req.params;
    try {
        ...
        const { name, description, size, price, tag, favorite, seller_id, bought_id } = req.body;
